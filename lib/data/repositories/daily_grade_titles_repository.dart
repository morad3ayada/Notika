...
class DailyGradeTitlesRepository {
  ...
  Future<DailyGradeTitlesResponse> getDailyGradeTitles({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          for (int i = 0; i < titlesData.length; i++) {
            try {
              ...
              titles.add(title);
              print('✅ تم تحويل العنوان ${i + 1}: ${title.displayTitle} (maxGrade: ${title.maxGrade})');
            } catch (e) {
          ...
          print('✅ تم جلب ${titles.length} عنوان درجة بنجاح من أصل ${titlesData.length}');
          ...
          try {
            ...
            return DailyGradeTitlesResponse.error('خطأ في تحليل بيانات عناوين الدرجات: ${e.toString()}');
          }
        }
        ...
        return DailyGradeTitlesResponse.error('انتهت صلاحية الجلسة، يرجى تسجيل الدخول مرة أخرى');
      } else if (response.statusCode == 403) {
        return DailyGradeTitlesResponse.error('ليس لديك صلاحية للوصول لعناوين الدرجات');
      } else if (response.statusCode == 404) {
        return DailyGradeTitlesResponse.error('لم يتم العثور على عناوين درجات لهذا الفصل');
      } else if (response.statusCode >= 500) {
        return DailyGradeTitlesResponse.error('خطأ في الخادم، يرجى المحاولة لاحقاً');
      } else {
        return DailyGradeTitlesResponse.error('حدث خطأ غير متوقع (${response.statusCode})');
      }
    } catch (e) {
      ...
      if (e.toString().contains('SocketException') || e.toString().contains('NetworkException')) {
        ...
        return DailyGradeTitlesResponse.error('انتهت مهلة الاتصال، يرجى المحاولة مرة أخرى');
      } else if (e.toString().contains('FormatException')) {
        return DailyGradeTitlesResponse.error('خطأ في تنسيق البيانات المُستلمة');
      } else {
        return DailyGradeTitlesResponse.error('حدث خطأ أثناء جلب البيانات: ${e.toString()}');
      }
    }
  }

  /// البحث عن عنوان درجة معين (اختياري للمستقبل)
  Future<DailyGradeTitlesResponse> searchGradeTitles({
    ...
    try {
      ...
      return DailyGradeTitlesResponse.error('حدث خطأ أثناء البحث: ${e.toString()}');
    }
  }
